...
class GitHubPortfolioAnalyzer
    ...
    def analyze_repositories(self)
        ...
        for repo in self.user.get_repos()
            ...
            try
                ...
                print(fErreur lors de l'analyse de {repo.name} {str(e)})
    ...
    def generate_markdown_report(self, portfolio, filename='PORTFOLIO.md')
        with open(filename, 'w', encoding='utf-8') as f
            f.write('# Portfolio des Projets GitHubnn')
            
            f.write('## Vue d'ensemblenn')
            f.write(fNombre total de repositories {len(portfolio['repositories'])}n)
            f.write(fNombre total de TODOs {len(portfolio['todos'])}nn)
            
            f.write('## Repositoriesnn')
            for repo in portfolio['repositories']
                f.write(f### {repo['name']}nn)
                f.write(f- Description {repo['description'] or 'Pas de description'}n)
                f.write(f- Langage principal {repo['language'] or 'Non spécifié'}n)
                if repo['dependencies']
                    f.write(- Dépendancesn)
                    for dep in repo['dependencies']
                        f.write(f  - {dep}n)
                f.write(f- [Voir sur GitHub]({repo['url']})nn)
                
                if repo['todos']
                    f.write(#### TODOsn)
                    for todo in repo['todos']
                        f.write(f- {todo['title']}n)
                    f.write(n)
            
            f.write('## TODOs Globauxnn')
            for todo in portfolio['todos']
                f.write(f- [{todo['repository']}] {todo['title']}n)
